...
export default function BookBasicInfo({loading, data}) {
    ...
    useEffect(() => {
        axios.get(`/api/fav/existItem/book/${cookie.load('userId')}/${params.id}`)
            ...
            >
                ...
                <Grid md={6}>
                    <Typography variant="body1">
                        {loading ? <Skeleton/> : `ISBN：${data.isbn}`}
                    </Typography>
                </Grid>
                <Grid md={6}>
                    <Typography variant="body1">
                        {loading ? <Skeleton/> : `作者：${data.writer}`}
                    </Typography>
                </Grid>
                <Grid md={6}>
                    <Typography variant="body1">
                        {loading ? <Skeleton/> : `出版时间：${data.publicationDate}`}
                    </Typography>
                </Grid>
                <Grid md={6}>
                    <Typography variant="body1">
                        {loading ? <Skeleton/> : `出版社：${data.publishingHouse}`}
                    </Typography>
                </Grid>
