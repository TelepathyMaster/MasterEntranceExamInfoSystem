...
export default function InfoChangeDialog({loginOpen, handleLoginClose, handleSnackbarOpen, setIsLoggedIn}) {
            ...
            PaperProps={{
                ...
                onSubmit: (event) => {
                    ...
                    if (objData.password !== objData.passwordRepeat) {
                        ...
                        axios.put('/api/user', objData)
                            .then(function (response) {
                                if (response.data) {
                                    handleClose();
                                    handleSnackbarOpen('success', '修改成功，已重新登录', 1000);
                                    setIsLoggedIn(true);
                                    cookie.save('userId', response.data.id, {path: '/'});
                                    cookie.save('userType', response.data.type, {path: '/'});
